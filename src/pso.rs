    ...
    O: ObjectiveFunction
{
    pub fn new(
        ...
        let mut velocities = Vec::with_capacity(swarm_size);
        ...
        for i in 0..swarm_size {
            // Generate random particle within bounds
            let particle = Array::random(dim, Uniform::new(lower_bound, upper_bound));
            let velocity = Array::random(dim, Uniform::new(lower_bound, upper_bound));

            let objective_value = objective.cost(&particle);

            particles.push(particle.clone());
            velocities.push(velocity);
            particle_bests.push((particle.clone(), objective_value));

            // Setting the initial global_best
    ...
    fn step(&mut self) {
        ...
        for i in 0..self.swarm_size {
            ...
            let cognitive_update =
                (p_best_x - particle) * self.cognitive_factor * rand::random::<f64>();
            let social_update =
                (&self.global_best_x - particle) * self.social_factor * rand::random::<f64>();

            let updated_velocity = velocity * self.inertia + cognitive_update + social_update;
            let updated_velocity =
                Self::restrict_particle(updated_velocity, self.v_min, self.v_max);

            let updated_particle = particle + &updated_velocity;
            let updated_particle =
                Self::restrict_particle(updated_particle, self.lower_bound, self.upper_bound);
            ...
            let objective_value = self.objective.cost(&updated_particle);
